    ...
    private Vector2[] _pathPositions = new Vector2[]
{
    ...
    [SerializeField]
    private Vector2[] _specialTilePositions = new Vector2[]
    {
        new Vector2(3,0), // Special
        new Vector2(0,3), // Event
        new Vector2(2,0)  // Checkpoint
    };
    ...
    {
        // Special tiles setup (this is just an example, customize as needed)
        if (_pathTiles.Count > 2)
            _pathTiles[2].tileType = Tile.TileType.Checkpoint;
        _pathTiles[2].SetColor(_checkpointColor);

        if (_pathTiles.Count > 5)
            _pathTiles[5].tileType = Tile.TileType.Event;
        _pathTiles[5].SetColor(_eventTileColor);

        if (_pathTiles.Count > 9)
            _pathTiles[9].tileType = Tile.TileType.Special;
        _pathTiles[9].SetColor(_specialTileColor);

        // Process special tile positions array if provided
        foreach (var specialPos in _specialTilePositions)
        {
            // Find the closest tile to this position
            Tile closestTile = GetTileAtPosition(specialPos);
            if (closestTile != null)
            {
                // You can customize this logic to determine different types of special tiles
                closestTile.SetColor(_specialTileColor);
                closestTile.tileType = Tile.TileType.Special;
            }
        }
    }
